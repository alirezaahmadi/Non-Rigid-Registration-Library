...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
//  optimizations...
// 1. jacobianT matricescan be generate in kernel at the smae time 
#include "nonRigidICP.h"
// #define DEBUG
// #define DEBUG_DefGraph
// #define LOG_EN
#define LOG_MAX 100
namespace DynaMap{
namespace solver{
    ...
    float nonRigidICP::getDQGradient(math::dualQuat *subWarpField,
                                     ...
                                     float3 vertexPose){
        // for(int j=0; j< KNN; j++){
        //     printf("id: %d, %f", j,subWarpFiledWeights[j]);
        // }                               
        ...
        float3 blendedNDQ = blender.blendVertexPose(subWarpField, subWarpFiledWeights, vertexPose);
        // change i-th (paramID) in +EPSILLON value
        switch(paramID){
            case 0:{
                // roll -> x
        ...
        subWarpField[nodeIndex] = math::dualQuat(euAngles, trans);
        // blend vertex with k neighbour 
        float3 blendedPDQ = blender.blendVertexPose(subWarpField, subWarpFiledWeights, vertexPose);
        // get numerical derivative w.r.t the changed parameter
        float nGradient = distance(blendedPDQ , blendedNDQ)/(2 * EPSILLON);
    ...
    void nonRigidICP::updateDataJacobianBlock(Jacobian *_Jacob, 
        ...
        int m = ObsNum;
        int id = (nodeID * m * 6) + ObsID;
        // Filling Jacobian Blocks 
        for(int paramID = 0; paramID < 6; paramID++){
            int jacobIndex = paramID * m + id;
            _Jacob[jacobIndex] = getDQGradient(subWarpField, 
                                               subWarpFiledWeights,
                                               blender, 
                                               nodeIndex, 
                                               paramID, 
                                               srcVertexNormal, 
                                               srcVertexPose);
            // if(_Jacob[jacobIndex] != 0.0f)printf("Jacob: %d, %f \n",jacobIndex, _Jacob[jacobIndex]);
        }
    }
    ...
    void buildDataJacbianKernel(nonRigidICP &nonRigidSolver,
        ...
        for (int idx = index; idx < size; idx += stride) {
            ...
            if(sourcedepth[idx] > 2.0 || sourcedepth[idx] < 0.03 ||
               targetdepth[idx] > 2.0 || targetdepth[idx] < 0.03 || cloud.normals[idx] == make_float3(0,0,0)) continue;

            // project predicted image pixels to 3D-space
            float3 vc = getPoint3d(idx, sourcedepth[idx], sensor); // todo... make sure transform is correct 
            float3 nc = cloud.normals[idx];    // todo ....update normals....
            // get the corresponding vertex in live-frame depth image 
            float3 vl = cuPose * getPoint3d(idx, targetdepth[idx], sensor); // todo... make sure transform is correct 
            // update residuals
            ...
            for(int nodeIndex = 0; nodeIndex < KNN; nodeIndex++){
                ...
                //                                        sensor.rows * sensor.cols,   // Observations Num
                //                                        idx,                         // pixel ID
                //                                        nodeID,
        ...
        for(int pylvl = 2; pylvl < 3; pylvl++) {
            ...
            for (iterationNum = 1 ; iterationNum < Prop.lvlIterationNum[pylvl]; iterationNum++) {
                ...
                // write regJacob into file for debug
                    ...
                    n = 6 * warpField.nodeNum;
                    writeMatrixToTxt("../logs/regJacob.txt", regJacob, sizeof(Jacobian), m, n, 1, 0, LOG_MAX);
                #endif
                // write rReg into file for debug
                #ifdef DEBUG
                    m = 3 * warpField.nodeNum;
                    n = 1;
                    writeMatrixToTxt("../logs/rReg.txt", rReg, sizeof(Jacobian), m, n, 1, 0, LOG_MAX);
                #endif
                #ifdef LOG_EN
                    std::cout << "buildLinearSystem... " << std::endl;
                #endif
                ...
                for(int cnt = 0; cnt < warpField.nodeNum; cnt++){
                    if(isnan(rData[cnt]))continue;
                    currError += pow(rData[cnt], 2);
                }
                for(int cnt = 0; cnt < warpField.nodeNum * 3; cnt++){
                    if(isnan(rReg[cnt]))continue;
                    currError += LAMBDA * pow(rReg[cnt], 2);
                }
    ...
    void nonRigidICP::solve(geometry::defGraph &warpField,
        ...
        for(int pylvl = 2; pylvl < 3; pylvl++) {
            ...
            for (iterationNum = 1 ; iterationNum < Prop.lvlIterationNum[pylvl]; iterationNum++) {
                ...
                // for(int cnt=0;cnt<warpField.nodeNum; cnt++){
                //     std::cout << "index: " << cnt << ", " << warpField.nodes[cnt].dq << std::endl;
                // }
                // for(int cnt = 0; cnt < targetMesh.verticesNum; cnt ++){
                //     for(int j =0; j<warpField.nNum; j++){
                //         int nidx = cnt * warpField.nNum + j;
                //         printf("id: %d, j:%d,  %d, %f, %f \n", cnt, j , warpField.visibleNodeIds[nidx], warpField.visibleNDistances[nidx], warpField.visibleNWeights[nidx]);
                //     }
                // }
                buildDataJacbianKernel<<<thread_blocks, threads_per_block>>>(*this,
                                                                             warpField,
    ...
    void nonRigidICP::cuSolveLinearSystem(geometry::defGraph &warpField){
        ...
        #ifdef DEBUG
            writeMatrixToTxt("../logs/result.txt", result, sizeof(Jacobian), m, n, 1, 0, m);
        #endif    
    }
    void nonRigidICP::solveLinearSystem(geometry::defGraph &warpField){
        ...
        cuBlasVectorSum(_alfa, result, _beta, b, result, m);
        // std::cout << "updating WarpField ... " << std::endl;
